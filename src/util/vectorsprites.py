...
class VectorSprite:
    ...
    def __init__(self, position, heading, pointlist, angle=0, color=(255, 255, 255), scale=1):
    ...
    def rotateAndTransform(self):
        ...
        #self.transformedPointlist = [self.scalePoint(point) for point in newPointList]
    ...
    def scale(self, point):
        newPoint = []
        newPoint.append(point[0] * self.scale)
        newPoint.append(point[1] * self.scale)
        # Keep points as integers
        newPoint = [int(point) for point in newPoint]
        return newPoint
